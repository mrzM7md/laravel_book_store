...
@section('content')
<button onclick="showAddNewCat()" class="btn btn-primary"><i class="fas fa-plus"></i> أضف مؤلفًا جديدًا</button>
<div style="display: none" id="create-author">
        ...
        <table id="books-table" class="table table-stribed text-right" width="100%" cellspacing="0">
            ...
            <tbody>
                 @foreach ($authors as $author)
                    <tr id="tr-{{$author->id}}">
                        <td>
                            @include('admin.authors.edit', $author)
                            @include('admin.authors.destroy', $author)
                        </td>
                    </tr>
                @endforeach
            </tbody>
...
@section('scripts')
    ...
    <script>
        function showAddNewCat(){
            if (document.getElementById("create-author").style.display == "none")
                document.getElementById("create-author").style.display = "flex";
            else
                document.getElementById("create-author").style.display = "none";
        }
    </script>
