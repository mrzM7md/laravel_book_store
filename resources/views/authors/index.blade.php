...
<section class="search-section text-center mb-4 row mx-auto">
    ...
    <div class="serach-box text-center shadow p-2 my-1 bg-body rounded">    
        <form action="{{ route('authors.search', ['slug' => $slug]) }}" method="GET" class="text-center col-12 row" role="search">
...
<section class="all-products-view">
    <div class="text-center">
      <h2>الكتب التابعة للمؤلف {{$authorName}}</h2>
      @if ($keyword)
        <p>عرض نتائج البحث حول كلمة <span>{{$keyword}}</span></p>
      @endif
    </div>

    @php $autocompleteRoute = route('authors.autocomplete', ['slug' => $slug]) @endphp
